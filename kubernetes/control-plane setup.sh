# Step 1: Disable Swap using the below commands

swapoff -a
sudo sed -i '/ swap / s/^\(.*\)$/#\1/g' /etc/fstab
  
# Step 2: Forwarding IPv4 and letting iptables see bridged traffic
...
curl -LO https://github.com/containerd/containerd/releases/download/v2.1.4/containerd-2.1.4-linux-arm64.tar.gz
...
# Step 4: Install runc
curl -LO https://github.com/opencontainers/runc/releases/download/v1.3.1/runc.arm64
...
sudo tar Cxzvf /opt/cni/bin cni-plugins-linux-arm64-v1.8.0.tgz
...
# Perform the below steps on the worker nodes
